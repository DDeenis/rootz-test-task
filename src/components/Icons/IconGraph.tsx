import React from "react";

export const IconGraph = () => {
  return (
    <svg
      width="41"
      height="40"
      viewBox="0 0 41 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M20.1396 0.000244141H22.1396C32.0656 0.000244141 40.1396 8.07853 40.1396 18.0009V20.001H20.1396V0.000244141ZM16.1396 4.12134V24H36.0156L35.9837 24.2441C34.8616 33.2264 27.1916 39.9986 18.1396 39.9986C8.21365 39.9986 0.139648 31.9243 0.139648 21.998C0.139648 12.9477 6.91165 5.27538 15.8916 4.15334L16.1396 4.12134Z"
        fill="black"
      />
    </svg>
  );
};
